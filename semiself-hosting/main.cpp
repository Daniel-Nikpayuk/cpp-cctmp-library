...
// square:
	...
	// test:
		...
		constexpr auto square(size_type const v)
		{
			...
			using eval_method_type = resolve_eval_method<concord_type, 10, 10, 10>;
			...
			auto ring8_to_ring8_f  = eval_method.declare_type({ ring8_t, ring8_t });
			auto func_sign         = eval_method.define_abstract(ring8_to_ring8_f, eval_contr::result);
			eval_method            . apply(func_sign, {v});

			return eval_method.rtn_cat(0);
		}
...
// sum of squares:
	...
	// test:
		...
		constexpr auto sum_of_squares(size_type const v1, size_type const v2)
		{
			...
			auto ring8_t           = ring_method.declare_type(8);
			auto r8_to_r8_to_r8_f  = eval_method.declare_type({ ring8_t, ring8_t, ring8_t });
			auto func_sign         = eval_method.define_abstract(r8_to_r8_to_r8_f, eval_contr::result);
			eval_method            . apply(func_sign, {v1, v2});
