		...
		(
			...
			// asm:
				...
				// op value:
					...
					sxa_pair( "asm_op_morph" , ActName::asm_op_morph ),
					sxa_pair( "asm_op_cycle" , ActName::asm_op_cycle ),
				...
				// arg value:
					...
					sxa_pair( "asm_arg_morph" , ActName::asm_arg_morph ),
					sxa_pair( "asm_arg_cycle" , ActName::asm_arg_cycle ),
...
#include NIK_CHORD_ASSEMBLY_PARSER_OBJ
	...
	struct T_chord_assembly_pda : public generator::T_generic_pda<T_grammar>
	{
		...
		nik_ces auto prod_size		= 7;//cctmp::string_literal("<A|C|M|L>").size();
